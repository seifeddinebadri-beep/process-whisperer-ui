...
serve(async (req) => {
  ...
  try {
    ...
    // 3. Call AI to generate automation use cases
        ...
        messages: [
          {
            ...
            content:
              ...
              "Pour chaque cas d'usage, évalue la complexité, l'impact et le ROI potentiel. " +
              "Retourne tes résultats UNIQUEMENT via l'appel de fonction fourni.",
          },
          {
            role: "user",
            content: `Analyse ce processus et génère des cas d'usage d'automatisation :\n\n${fullContext}`,
          },
        ],
        tools: [
          {
            ...
            function: {
              ...
              description: "Store the generated automation use cases",
              parameters: {
                type: "object",
                properties: {
                  use_cases: {
                    ...
                    items: {
                      ...
                      properties: {
                        ...
                        description: { type: "string", description: "Description détaillée de l'automatisation proposée" },
                        complexity: { type: "string", enum: ["low", "medium", "high"], description: "Complexité de mise en œuvre" },
                        impact: { type: "string", enum: ["low", "medium", "high"], description: "Impact business attendu" },
                        roi_estimate: { type: "string", description: "Estimation du ROI (ex: '30% réduction temps de traitement')" },
                        tools_suggested: {
                          type: "array",
                          items: { type: "string" },
                          description: "Outils ou technologies suggérés (ex: 'UiPath', 'Power Automate', 'GPT-4')",
                        },
                      },
                      required: ["title", "description", "complexity", "impact", "roi_estimate", "tools_suggested"],
    ...
    if (!Array.isArray(use_cases) || use_cases.length === 0) {
      throw new Error("No use cases generated");
    }

    // 4. Delete existing use cases for this process (re-analysis)
    await supabase.from("automation_use_cases").delete().eq("process_id", process_id);

    // 5. Insert new use cases
    const inserts = use_cases.map((uc: any) => ({
      process_id,
      title: uc.title,
    ...
    const { error: insertError } = await supabase.from("automation_use_cases").insert(inserts);
    if (insertError) {
      throw new Error(`Failed to insert use cases: ${insertError.message}`);
    }

    // 6. Update process status to "approved"
