...
export const mockUseCaseDetails: Record<string, UseCaseDetail> = {
  ...
  // ---------- uc7: Gestion Tickets Téléphoniques ITSM ----------
  uc7: {
    useCaseId: "uc7",
    linkedProcess: "Gestion Tickets Téléphoniques",
    department: "IT Service Management",
    entity: "Support DGP",
    automationPattern: "Form Auto-Fill & Cross-System Sync",

    detectionSignals: [
      {
        id: "ds7-1",
        type: "repetitive_manual",
        label: "Saisie manuelle répétitive",
        description: "L'utilisateur remplit manuellement 20+ champs dans le formulaire OTOBO pour chaque ticket téléphonique. 1 324 actions de saisie observées.",
        triggeringStep: "Saisie des champs du ticket",
        ruleOrThreshold: "45 actions de saisie par ticket × fréquence élevée",
      },
      {
        id: "ds7-2",
        type: "tool_transfer",
        label: "Transfert inter-applications",
        description: "Copier-coller fréquent entre le CRM SharePoint Excel et le formulaire OTOBO ITSM via Edge.",
        triggeringStep: "Consultation et mise à jour du CRM",
        ruleOrThreshold: "Allers-retours CRM↔OTOBO détectés (637 occurrences)",
      },
      {
        id: "ds7-3",
        type: "high_frequency",
        label: "Volume élevé",
        description: "847 actions observées au total sur le processus. L'étape de saisie seule représente 1 324 actions avec des boucles de modification.",
        triggeringStep: "Saisie des champs du ticket",
        ruleOrThreshold: "Fréquence > 100 exécutions/jour",
      },
      {
        id: "ds7-4",
        type: "structured_inputs",
        label: "Champs structurés et prédéfinis",
        description: "La majorité des champs du ticket (CodeSite, Département, TypeBailleur, Canal, Région, MMO) sont des listes déroulantes ou des codes structurés.",
        triggeringStep: "Saisie des champs du ticket",
        ruleOrThreshold: "80% des champs sont des sélections dans des listes pré-définies",
      },
      {
        id: "ds7-5",
        type: "rule_based",
        label: "Décisions basées sur des règles",
        description: "Le routage entre saisie, consultation CRM et validation suit des patterns prédictibles observés dans les transitions.",
        triggeringStep: "Validation et soumission du ticket",
        ruleOrThreshold: "Transitions déterministes : S02→S03 (637x), S04→S05 (841x)",
      },
    ],

    contextReferences: [
      { id: "cr7-1", type: "role", name: "Agent Support DGP", detail: "Responsable de la création et gestion des tickets téléphoniques. Passe ~70% du temps sur la saisie manuelle." },
      { id: "cr7-2", type: "tool", name: "OTOBO ITSM 11", detail: "Système de ticketing principal (supportdgptest.qloudwise.com et supportdgp.amaris.com)." },
      { id: "cr7-3", type: "tool", name: "SharePoint Excel CRM", detail: "Fichier CRM partagé sur SharePoint (o2fitcom-my.sharepoint.com) utilisé pour consulter/mettre à jour les données client." },
      { id: "cr7-4", type: "tool", name: "Outlook", detail: "Consultation de tickets via notifications email (outlook.office.com)." },
      { id: "cr7-5", type: "tool", name: "Microsoft Edge", detail: "Navigateur utilisé pour accéder à toutes les applications web." },
      { id: "cr7-6", type: "constraint", name: "Multi-environnements OTOBO", detail: "Deux instances OTOBO utilisées (test et production), nécessitant une gestion cohérente." },
    ],

    confidenceLevel: "high",
    confidenceExplanation: "Confiance élevée : 847 actions observées, champs structurés, transitions répétitives et prévisibles, 5 applications identifiées.",

    willBeAutomated: [
      { step: "Ouverture du formulaire de nouveau ticket", system: "Robot RPA / API OTOBO" },
      { step: "Pré-remplissage des champs structurés", system: "Mapping CRM → OTOBO", decision: "Auto-sélection CodeSite, Département, Canal, Région" },
      { step: "Consultation CRM SharePoint", system: "Microsoft Graph API / Excel Online API" },
      { step: "Synchronisation données CRM ↔ Ticket", system: "Power Automate / Azure Functions" },
      { step: "Soumission du ticket", system: "OTOBO API / RPA submit" },
    ],

    willRemainManual: [
      { item: "Saisie du contenu descriptif libre (texte riche)", reason: "Le corps du ticket nécessite une description contextuelle que l'agent rédige pendant l'appel." },
      { item: "Gestion des exceptions et tickets verrouillés", reason: "L'investigation des tickets verrouillés requiert un jugement humain (841 consultations observées)." },
      { item: "Décision de routage entre modification, consultation CRM ou validation", reason: "L'agent décide en temps réel selon l'appel en cours." },
    ],

    explicitExclusions: [
      { scenario: "Tickets avec données client non référencées dans le CRM", reason: "Nécessite une recherche manuelle et potentiellement la création d'une fiche client." },
      { scenario: "Tickets multi-sites nécessitant plusieurs codes site", reason: "Logique de splitting non standard, requiert intervention humaine." },
      { scenario: "Modifications de tickets existants via recherche dans tickets verrouillés", reason: "Processus de recherche et modification trop variable (boucles de 841 occurrences)." },
    ],

    detailedSteps: [
      {
        id: "das7-1",
        stepName: "Réception de l'appel et identification appelant",
        trigger: "Appel téléphonique entrant détecté par le système CTI",
        inputData: ["Numéro de téléphone appelant", "Données CRM associées"],
        validationRules: ["Appelant identifié dans le CRM", "Code site valide"],
        systemAction: "Le système identifie l'appelant via le numéro de téléphone, récupère les données CRM depuis SharePoint et pré-remplit les champs du formulaire OTOBO.",
        outputProduced: "Formulaire de ticket pré-rempli avec : Nom demandeur, Code Site, Département, Type de bailleur, Région",
        logging: "Log : appel reçu, appelant identifié/non identifié, champs pré-remplis.",
      },
      {
        id: "das7-2",
        stepName: "Création automatique du ticket OTOBO",
        trigger: "Formulaire pré-rempli validé ou appel en cours",
        inputData: ["Données CRM pré-chargées", "Sélections de l'agent (Canal, Type demandeur, Statut)"],
        validationRules: ["Tous les champs obligatoires renseignés", "Code CRM valide", "Numéro DAS au format attendu"],
        systemAction: "Création du ticket via API OTOBO ou soumission RPA du formulaire avec les champs pré-remplis. Les champs dynamiques (dates, statut) sont configurés automatiquement.",
        outputProduced: "Ticket OTOBO créé avec numéro de référence",
        logging: "Log : ticket créé, numéro de référence, champs auto-remplis vs manuels, durée de création.",
      },
      {
        id: "das7-3",
        stepName: "Synchronisation bidirectionnelle CRM",
        trigger: "Ticket créé ou modifié dans OTOBO",
        inputData: ["Numéro de ticket", "Code CRM", "Données du ticket"],
        validationRules: ["Code CRM existe dans le fichier SharePoint", "Pas de conflit de version sur le fichier Excel"],
        systemAction: "Mise à jour automatique du fichier CRM Excel sur SharePoint avec les informations du ticket (code couleur, lien ticket, date).",
        outputProduced: "Fichier CRM mis à jour, lien de ticket copié",
        logging: "Log : CRM mis à jour, cellule modifiée, version du fichier, horodatage.",
      },
      {
        id: "das7-4",
        stepName: "Notification et confirmation",
        trigger: "Ticket créé et CRM synchronisé",
        inputData: ["Résumé du ticket", "Destinataires"],
        validationRules: ["Email de confirmation envoyé dans les 2 minutes"],
        systemAction: "Envoi automatique d'une notification email via Outlook avec le résumé du ticket et le lien direct vers OTOBO.",
        outputProduced: "Email de confirmation envoyé au demandeur et à l'équipe concernée",
        logging: "Log : notification envoyée, destinataires, horodatage.",
      },
    ],
    ...
    exceptions: [
      {
        id: "ex7-1",
        whatCanGoWrong: "Appelant non trouvé dans le CRM SharePoint",
        howDetected: "Recherche par numéro/nom ne retourne aucun résultat",
        whatHappensNext: "Le formulaire s'ouvre sans pré-remplissage. L'agent saisit manuellement et un flag 'nouveau contact' est posé.",
        whoIsNotified: "Agent support + Responsable CRM",
        dataPreserved: "Données saisies manuellement, tentatives de recherche CRM",
        severity: "medium",
      },
      {
        id: "ex7-2",
        whatCanGoWrong: "Fichier CRM Excel verrouillé par un autre utilisateur",
        howDetected: "Erreur de verrouillage SharePoint lors de la tentative de mise à jour",
        whatHappensNext: "La synchronisation CRM est mise en file d'attente. Réessai automatique toutes les 2 minutes pendant 30 min.",
        whoIsNotified: "Agent support (notification de délai)",
        dataPreserved: "Données de mise à jour en file d'attente, log des tentatives",
        severity: "low",
      },
      {
        id: "ex7-3",
        whatCanGoWrong: "OTOBO ITSM indisponible ou timeout",
        howDetected: "Timeout API après 3 tentatives ou erreur HTTP 5xx",
        whatHappensNext: "Données du ticket sauvegardées localement. Création automatique dès le rétablissement du service.",
        whoIsNotified: "Agent support + Équipe IT Operations",
        dataPreserved: "Formulaire complet en cache local, log des erreurs API",
        severity: "high",
      },
      {
        id: "ex7-4",
        whatCanGoWrong: "Incohérence entre les deux environnements OTOBO (test vs production)",
        howDetected: "Vérification automatique du code site et des champs dynamiques entre les deux instances",
        whatHappensNext: "Alerte levée, ticket créé uniquement en production. Synchronisation test reportée.",
        whoIsNotified: "Administrateur OTOBO",
        dataPreserved: "Diff des champs entre les deux environnements",
        severity: "medium",
      },
    ],
    ...
    comparison: {
      before: {
        steps: 6,
        humanEffort: "~8-12 minutes par ticket, 45+ actions de clic/saisie",
        toolsInvolved: ["OTOBO ITSM (2 instances)", "SharePoint Excel CRM", "Outlook", "Edge"],
        errorRisks: [
          "Erreurs de saisie sur 20+ champs manuels",
          "Données CRM non synchronisées (copier-coller)",
          "Oubli de mise à jour du fichier CRM après création du ticket",
          "Boucles de modification répétitives (1 324 actions de saisie observées)",
          "Consultation inefficace des tickets verrouillés (841 actions)",
        ],
      },
      after: {
        steps: 4,
        automationCheckpoints: [
          "Identification automatique de l'appelant via CRM",
          "Pré-remplissage des champs structurés (80% des champs)",
          "Synchronisation bidirectionnelle CRM automatique",
          "Notification automatique post-création",
        ],
        humanTouchpoints: [
          "Rédaction du contenu descriptif du ticket",
          "Validation finale avant soumission",
          "Gestion des exceptions (appelant inconnu)",
        ],
        residualRisks: [
          "Pré-remplissage incorrect si données CRM obsolètes",
          "Verrouillage fichier Excel CRM (délai de synchro)",
          "Changements d'interface OTOBO cassant l'automatisation RPA",
        ],
      },
    },
    ...
    valueMetrics: [
      {
        metric: "Valeur métier",
        level: "high",
        explanation: "Élimine ~75% de la saisie manuelle par ticket. Sur 847 actions observées, plus de 600 sont automatisables. Gain estimé : 6-8 minutes par ticket.",
        assumptions: "Basé sur l'analyse de 847 actions observées avec 1 324 actions de saisie et 637 consultations CRM.",
      },
      {
        metric: "Complexité d'implémentation",
        level: "medium",
        explanation: "Nécessite l'intégration de 5 applications (OTOBO, SharePoint, Outlook, Edge, CRM Excel). L'approche RPA est plus rapide mais l'intégration API est plus robuste.",
        assumptions: "Accès aux API OTOBO REST disponible. Fichier CRM Excel reste le format de référence à court terme.",
      },
      {
        metric: "Niveau de risque",
        level: "medium",
        explanation: "Risque modéré lié à la fragilité de l'approche RPA face aux changements d'UI OTOBO et au verrouillage du fichier CRM Excel partagé.",
        assumptions: "OTOBO ne subit pas de mises à jour majeures d'UI dans les 6 prochains mois.",
      },
      {
        metric: "Impact sur le changement",
        level: "low",
        explanation: "Les agents continuent d'utiliser les mêmes outils. Le formulaire est simplement pré-rempli. Formation minimale requise (30 min).",
        assumptions: "L'interface OTOBO reste identique. Les agents sont familiers avec le processus actuel.",
      },
    ],

    traceabilityLinks: [
      { type: "process_step", name: "S01 - Ouverture formulaire", detail: "Ouverture de la page de création de ticket téléphonique dans OTOBO ITSM 11 (148 occurrences)" },
      { type: "process_step", name: "S02 - Saisie des champs", detail: "Remplissage des 20+ champs du formulaire (1 324 actions, étape la plus lourde)" },
      { type: "process_step", name: "S03 - Consultation CRM", detail: "Consultation et mise à jour du fichier CRM Excel SharePoint (637 occurrences)" },
      { type: "process_step", name: "S04 - Validation et soumission", detail: "Soumission finale du ticket dans OTOBO (70 occurrences)" },
      { type: "process_step", name: "S05 - Tickets verrouillés", detail: "Consultation et recherche dans les tickets verrouillés (841 occurrences)" },
      { type: "process_step", name: "S06 - Consultation Outlook", detail: "Consultation de ticket via Outlook (1 occurrence)" },
      { type: "role", name: "Agent Support DGP", detail: "Opérateur principal du processus de création de tickets téléphoniques" },
      { type: "tool", name: "OTOBO ITSM 11", detail: "Système ITSM pour la gestion des tickets (2 instances : test et production)" },
      { type: "tool", name: "SharePoint Excel CRM", detail: "Fichier CRM partagé pour les données client" },
      { type: "tool", name: "Outlook", detail: "Client email pour les notifications de tickets" },
      { type: "tool", name: "Microsoft Edge", detail: "Navigateur pour accéder à OTOBO, SharePoint et Outlook" },
      { type: "business_rule", name: "Boucle Saisie-CRM", detail: "Allers-retours fréquents entre saisie ticket et consultation CRM (transition S02↔S03)" },
      { type: "business_rule", name: "Recherche tickets verrouillés", detail: "Post-soumission, recherche systématique dans les tickets verrouillés (S04→S05, 841 occurrences)" },
    ],
    ...
    decisions: [
      {
        id: "dec7-1",
        action: "approved",
        user: "Karim Benali (Responsable Support)",
        timestamp: "2026-03-02T10:30:00",
        comment: "Cas d'usage validé. La saisie manuelle est clairement le goulot d'étranglement. Prioriser l'approche RPA pour un quick win.",
      },
      {
        id: "dec7-2",
        action: "changes_requested",
        user: "Sophie Martin (IT Manager)",
        timestamp: "2026-03-02T14:00:00",
        comment: "Ajouter une étude de faisabilité pour l'intégration API OTOBO avant de se limiter au RPA. Vérifier la disponibilité de l'API REST.",
      },
    ],
    ...
    comments: [
      {
        ...
        text: "Les agents passent en moyenne 8-12 minutes par ticket juste pour la saisie. Avec 50+ tickets par jour, c'est plus de 6 heures de saisie pure. L'automatisation est critique.",
      },
      {
        id: "com7-2",
        user: "Sophie Martin (IT Manager)",
        timestamp: "2026-03-02T09:00:00",
        text: "Le fichier CRM Excel sur SharePoint est un point de friction majeur. À moyen terme, il faudrait migrer vers Dataverse ou un vrai CRM, mais le RPA peut déjà résoudre le problème de synchronisation.",
      },
      {
        id: "com7-3",
        user: "Ahmed Khelifi (Agent Support)",
        timestamp: "2026-03-02T11:30:00",
        text: "Le plus pénible c'est les allers-retours entre OTOBO et le fichier Excel. On copie-colle le code CRM, puis on revient dans OTOBO, et souvent on doit recopier car le champ s'est vidé.",
      },
    ],
  },
};
...
export const mockVariants: Record<string, MockVariant[]> = {
  ...
  uc7: [
    {
      variant_number: 1,
      variant_name: "RPA Bureau (UiPath)",
      approach_description: "Robot RPA qui réplique les actions utilisateur : ouverture du formulaire OTOBO, remplissage automatique des champs depuis un fichier source, consultation CRM SharePoint et soumission du ticket.",
      complexity: "medium",
      impact: "high",
      roi_estimate: "Réduction de 75% du temps de saisie",
      tools_suggested: ["UiPath", "OTOBO ITSM", "SharePoint Online", "Edge Browser"],
      pros: ["Pas de modification d'OTOBO", "Déploiement rapide", "Réplique le processus existant", "ROI immédiat"],
      cons: ["Fragile aux changements d'UI", "Maintenance régulière requise", "Licence UiPath", "Pas de transformation du processus"],
      estimated_cost: "15 000 – 30 000 €",
      estimated_timeline: "3-4 semaines",
      recommended: true,
    },
    {
      variant_number: 2,
      variant_name: "Intégration API OTOBO + CRM",
      approach_description: "Développement d'une intégration directe via les API REST d'OTOBO et Microsoft Graph pour créer les tickets programmatiquement et synchroniser les données CRM automatiquement.",
      complexity: "high",
      impact: "high",
      roi_estimate: "Réduction de 90% du temps + zéro erreur de saisie",
      tools_suggested: ["OTOBO REST API", "Microsoft Graph API", "Power Automate", "Azure Functions"],
      pros: ["Robuste et scalable", "Pas de dépendance UI", "Synchronisation temps réel CRM", "Traçabilité complète"],
      cons: ["Nécessite accès API OTOBO", "Développement plus long", "Compétences techniques requises", "Tests d'intégration complexes"],
      estimated_cost: "35 000 – 60 000 €",
      estimated_timeline: "6-8 semaines",
      recommended: false,
    },
    {
      variant_number: 3,
      variant_name: "Formulaire intelligent Power Apps",
      approach_description: "Remplacement du formulaire OTOBO par un formulaire Power Apps unifié qui pré-remplit les champs depuis le CRM, valide les données en temps réel et crée le ticket via API.",
      complexity: "medium",
      impact: "high",
      roi_estimate: "Réduction de 85% du temps + amélioration qualité données",
      tools_suggested: ["Power Apps", "Power Automate", "SharePoint", "OTOBO API", "Dataverse"],
      pros: ["Interface utilisateur moderne", "Validation temps réel", "Pré-remplissage CRM", "Mobile-ready"],
      cons: ["Licences Power Platform", "Formation utilisateurs", "Double maintenance possible", "Dépendance Microsoft"],
      estimated_cost: "20 000 – 40 000 €",
      estimated_timeline: "4-6 semaines",
      recommended: false,
    },
